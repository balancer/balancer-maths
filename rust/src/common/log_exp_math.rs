...
const MAX_NATURAL_EXPONENT: BigInt = BigInt::from(130000000000000000000i128);
const MIN_NATURAL_EXPONENT: BigInt = BigInt::from(-41000000000000000000i128);
const LN_36_LOWER_BOUND: BigInt = BigInt::from(900000000000000000i128);
const LN_36_UPPER_BOUND: BigInt = BigInt::from(1100000000000000000i128);
// Precomputed value of 2^254 / HUNDRED_WAD
const MILD_EXPONENT_BOUND: BigInt = BigInt::from_str("289480223093290488558927462521719769633174961664101410098").unwrap();

// RAY constant for 36 decimal precision
const RAY: BigInt = BigInt::from_str("1000000000000000000000000000000000000").unwrap();

// 18 decimal constants
const X0: BigInt = BigInt::from(128000000000000000000i128); // 2^7
const A0: BigInt = BigInt::from_str("38877084059945950922200000000000000000000000000000000000").unwrap(); // e^(x0) (no decimals)
const X1: BigInt = BigInt::from(64000000000000000000i128); // 2^6
const A1: BigInt = BigInt::from(6235149080811616882910000000i128); // e^(x1) (no decimals)

// 20 decimal constants
const X2: BigInt = BigInt::from(3200000000000000000000i128); // 2^5
const A2: BigInt = BigInt::from_str("7896296018268069516100000000000000").unwrap(); // e^(x2)
const X3: BigInt = BigInt::from(1600000000000000000000i128); // 2^4
const A3: BigInt = BigInt::from_str("888611052050787263676000000").unwrap(); // e^(x3)
const X4: BigInt = BigInt::from(800000000000000000000i128); // 2^3
const A4: BigInt = BigInt::from_str("298095798704172827474000").unwrap(); // e^(x4)
const X5: BigInt = BigInt::from(400000000000000000000i128); // 2^2
const A5: BigInt = BigInt::from_str("5459815003314423907810").unwrap(); // e^(x5)
const X6: BigInt = BigInt::from(200000000000000000000i128); // 2^1
const A6: BigInt = BigInt::from_str("738905609893065022723").unwrap(); // e^(x6)
const X7: BigInt = BigInt::from(100000000000000000000i128); // 2^0
const A7: BigInt = BigInt::from_str("271828182845904523536").unwrap(); // e^(x7)
const X8: BigInt = BigInt::from(50000000000000000000i128); // 2^-1
const A8: BigInt = BigInt::from_str("164872127070012814685").unwrap(); // e^(x8)
const X9: BigInt = BigInt::from(25000000000000000000i128); // 2^-2
const A9: BigInt = BigInt::from_str("128402541668774148407").unwrap(); // e^(x9)
const X10: BigInt = BigInt::from(12500000000000000000i128); // 2^-3
const A10: BigInt = BigInt::from_str("113314845306682631683").unwrap(); // e^(x10)
const X11: BigInt = BigInt::from(6250000000000000000i128); // 2^-4
const A11: BigInt = BigInt::from_str("106449445891785942956").unwrap(); // e^(x11)

const HUNDRED_WAD: BigInt = BigInt::from(100000000000000000000i128);
...
pub fn pow(x: &BigInt, y: &BigInt) -> Result<BigInt, PoolError> {
    ...
    if x >= &BigInt::from_str("57896044618658097711785492504343953926634992332820282019728792003956564819968").unwrap() {
...
fn exp(x: &BigInt) -> Result<BigInt, PoolError> {
    ...
    // All terms with n >= 10 are zero, as established by the quotient rule (when n >= 10, (x^n / n!) < 1).
    ...
    Ok((first_an * series_sum) / &HUNDRED_WAD)
}
