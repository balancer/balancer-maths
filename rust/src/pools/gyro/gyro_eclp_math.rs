...
use crate::pools::gyro::signed_fixed_point::{
    div_down_mag, div_down_mag_u, div_up_mag_u, div_xp_u, mul_down_mag, mul_down_mag_u,
    mul_down_xp_to_np_u, mul_up_fixed, mul_up_mag_u, mul_up_xp_to_np_u, mul_xp_u,
};
...
fn virtual_offset0(p: &EclpParams, d: &DerivedEclpParams, r: &Vector2) -> BigInt {
    ...
    a + mul_up_xp_to_np_u(&mul_up_mag_u(&r.x, &p.s), &div_xp_u(&d.tau_beta.y, &d.d_sq))
}
...
fn calc_at_a_chi(x: &BigInt, y: &BigInt, p: &EclpParams, d: &DerivedEclpParams) -> BigInt {
    ...
    let term_xp = div_xp_u(
        &div_down_mag_u(&(div_down_mag_u(&d.w, &p.lambda) + &d.z), &p.lambda),
        &d_sq2,
    );
    ...
    val += mul_down_xp_to_np_u(&term_np, &div_xp_u(&d.u, &d_sq2));

    // (sx+cy) * v, note v > 0
    let term_np = mul_down_mag_u(x, &p.s) + mul_down_mag_u(y, &p.c);
    val += mul_down_xp_to_np_u(&term_np, &div_xp_u(&d.v, &d_sq2));
...
fn calc_a_chi_a_chi_in_xp(p: &EclpParams, d: &DerivedEclpParams) -> BigInt {
    ...
    let mut val = mul_up_mag_u(
        &p.lambda,
        &div_xp_u(&mul_xp_u(&(&d.u * BigInt::from(2u64)), &d.v), &d_sq3),
    );
...
fn calc_min_atx_a_chiy_sq_plus_atx_sq(
    ...
    let term_xp3 = div_down_mag_u(&div_down_mag_u(&mul_xp_u(&d.v, &d.v), &p.lambda), &p.lambda);
    ...
    val += mul_down_xp_to_np_u(&term4, &term5);
...
fn calc_2_atx_aty_a_chix_a_chiy(
    ...
    let term_xp = mul_xp_u(&d.z, &d.u)
        + div_down_mag_u(&div_down_mag_u(&mul_xp_u(&d.w, &d.v), &p.lambda), &p.lambda);

    let term_xp =
        term_xp + div_down_mag_u(&(mul_xp_u(&d.w, &d.u) + mul_xp_u(&d.z, &d.v)), &p.lambda);
...
fn calc_min_aty_a_chix_sq_plus_aty_sq(
    ...
    let term_xp = mul_xp_u(&d.z, &d.z)
        + div_down_mag_u(&div_down_mag_u(&mul_xp_u(&d.w, &d.w), &p.lambda), &p.lambda);

    let term_xp =
        term_xp + div_down_mag_u(&mul_xp_u(&(&d.z * BigInt::from(2u64)), &d.w), &p.lambda);
...
pub fn calculate_invariant_with_error(
    ...
    let mul_denominator = div_xp_u(&_ONE_XP, &(&achiachi - &*_ONE_XP));
    ...
    let lambda_squared_div_1e36 = &(&params.lambda * &params.lambda)
        / BigInt::from_str("1000000000000000000000000000000000000").unwrap();
    let numerator = &mul_up_xp_to_np_u(&invariant, &mul_denominator)
...
fn solve_quadratic_swap(
    ...
    let lam_bar = Vector2 {
        x: &*_ONE_XP - div_down_mag_u(&div_down_mag_u(&_ONE_XP, lambda), lambda),
        y: &*_ONE_XP - div_up_mag_u(&div_up_mag_u(&_ONE_XP, lambda), lambda),
    };
    ...
    let s_term = Vector2 {
        x: div_xp_u(&mul_down_mag_u(&mul_down_mag_u(&lam_bar.y, s), s), d_sq),
        y: div_xp_u(
    ...
    q.c = &q.c + mul_down_xp_to_np_u(&mul_down_mag_u(&r.y, &r.y), &s_term_y);
    ...
    if &q.b - &q.c > BigInt::zero() {
        ...
        q.a = mul_up_xp_to_np_u(&(&q.b - &q.c), &div_xp_u(&_ONE_XP, &s_term_x));
    }
...
fn calc_xp_xp_div_lambda_lambda(
    ...
    if term_xp > BigInt::zero() {
        ...
        q.a = mul_up_xp_to_np_u(&mul_up_mag_u(&q.a, c), &(&term_xp + BigInt::from(7u64)));
    } else {
        q.a = mul_down_mag_u(&r.y, &r.y);
        q.a = mul_down_mag_u(&q.a, &(s * BigInt::from(2u64)));
        q.a = mul_up_xp_to_np_u(&mul_down_mag_u(&q.a, c), &term_xp);
    }
    ...
    let term_xp2 = div_xp_u(&mul_xp_u(&tau_beta.y, &tau_beta.y), &sq_vars.x) + BigInt::from(7u64);

    q.b = mul_up_mag_u(&sq_vars.y, s);
    q.b = mul_up_xp_to_np_u(&mul_up_mag_u(&q.b, s), &term_xp2);
    ...
    let term_xp2 = div_xp_u(&mul_xp_u(&tau_beta.x, &tau_beta.x), &sq_vars.x) + BigInt::from(7u64);

    let val = mul_up_mag_u(&mul_up_mag_u(&sq_vars.y, c), c);
    mul_up_xp_to_np_u(&val, &term_xp2) + &q.a
}

fn calc_y_given_x(x: &BigInt, params: &EclpParams, d: &DerivedEclpParams, r: &Vector2) -> BigInt {
    let ab = Vector2 {
...
fn calc_x_given_y(y: &BigInt, params: &EclpParams, d: &DerivedEclpParams, r: &Vector2) -> BigInt {
    let ba = Vector2 {
