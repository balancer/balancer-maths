...
pub fn compute_invariant(
    ...
    let mut prev_invariant = BigInt::zero();
    ...
    for _ in 0..255 {
        ...
        prev_invariant = invariant.clone();
...
pub fn compute_balance(
    ...
    for (i, balance) in balances.iter().enumerate() {
        if i != token_index {
            d_p = (d_p.clone() * invariant.clone()) / (balance.clone() * BigInt::from(num_tokens));
        }
    }
    
    let numerator = (amp_times_total.clone() * invariant.clone()) / BigInt::from(AMP_PRECISION);
    let numerator = numerator + (d_p.clone() * BigInt::from(num_tokens));
