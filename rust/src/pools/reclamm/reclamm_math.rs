...
pub fn compute_current_virtual_balances(
    ...
    if last_timestamp == current_timestamp {
        return (last_virtual_balance_a.clone(), last_virtual_balance_b.clone(), false);
    }
    ...
    if current_timestamp > price_ratio_update_start_time && last_timestamp < price_ratio_update_end_time {
        let (new_virtual_balance_a, new_virtual_balance_b) = calculate_virtual_balances_updating_price_ratio(
    ...
    if centeredness < *centeredness_margin {
        let (new_virtual_balance_a, new_virtual_balance_b) = compute_virtual_balances_updating_price_range(
            balances_scaled_18,
            &current_virtual_balance_a,
            &current_virtual_balance_b,
    ...
    (current_virtual_balance_a, current_virtual_balance_b, changed)
}
...
fn calculate_virtual_balances_updating_price_ratio(
    ...
    let (balance_token_undervalued, last_virtual_balance_undervalued, last_virtual_balance_overvalued) = if is_pool_above_center {
        (&balances_scaled_18[A], last_virtual_balance_a, last_virtual_balance_b)
    } else {
        (&balances_scaled_18[B], last_virtual_balance_b, last_virtual_balance_a)
    };
    ...
    let sqrt_input = &pool_centeredness * (&pool_centeredness + &(4 * &sqrt_price_ratio) - &*TWO_WAD) + &*RAY;
    ...
    let virtual_balance_overvalued = 
        (&virtual_balance_undervalued * last_virtual_balance_overvalued) / last_virtual_balance_undervalued;
...
fn compute_virtual_balances_updating_price_range(
    ...
    let shift_factor = pow(daily_price_shift_base, &time_difference_wad).unwrap_or_else(|_| WAD.clone());
    ...
    let denominator = mul_down_fixed(&price_ratio_minus_one, &virtual_balance_overvalued)
        .unwrap_or_else(|_| BigInt::zero()) - balances_scaled_overvalued;
    
    let virtual_balance_undervalued = 
        (balances_scaled_undervalued * (&virtual_balance_overvalued + balances_scaled_overvalued)) / &denominator;
...
fn compute_price_ratio(
    ...
    let (min_price, max_price) = compute_price_range(
        balances_scaled_18,
        virtual_balance_a,
        virtual_balance_b,
    );
...
fn compute_fourth_root_price_ratio(
    ...
    let minimum_fourth_root_price_ratio = if start_fourth_root_price_ratio < end_fourth_root_price_ratio {
...
fn compute_centeredness(
    ...
    if numerator <= denominator {
        let pool_centeredness = div_down_fixed(&numerator, &denominator).unwrap_or_else(|_| BigInt::zero());
        let is_pool_above_center = false;
        (pool_centeredness, is_pool_above_center)
    } else {
        let pool_centeredness = div_down_fixed(&denominator, &numerator).unwrap_or_else(|_| BigInt::zero());
...
pub fn compute_invariant(
    ...
    match rounding {
        Rounding::RoundDown => mul_down_fixed(&total_balance_a, &total_balance_b)
            .unwrap_or_else(|_| BigInt::zero()),
        Rounding::RoundUp => mul_up_fixed(&total_balance_a, &total_balance_b)
            .unwrap_or_else(|_| BigInt::zero()),
    }
}
...
pub fn compute_out_given_in(
    ...
    let current_total_token_out_pool_balance = &balances_scaled_18[token_out_index] + virtual_balance_token_out;
    ...
    let amount_out_scaled_18 = current_total_token_out_pool_balance - new_total_token_out_pool_balance;
...
pub fn compute_in_given_out(
    ...
    .unwrap_or_else(|_| BigInt::zero()) - &balances_scaled_18[token_in_index] - virtual_balance_token_in;
